/*
  ...
  - O submit do form deve ser feito para o endpoint abaixo. Atente-se para os 2
    ...
    - https://api.giphy.com/v1/gifs/search?api_key=sHOeTiUt4Z3KBCpskpcDkE54u1nj29nh&limit=1&q=[VALOR_INSERIDO_NO_INPUT]
...
form.addEventListener('submit', async event => {
  ...
  const APIKey = 'sHOeTiUt4Z3KBCpskpcDkE54u1nj29nh'
  ...
  try {
    ...
    const downsizeGifImgUrl = GIFData.data[0].images.downsized.url
    const titleGif = GIFData.data[0].title
    const img = document.createElement("img");

    img.setAttribute('src', downsizeGifImgUrl)
    img.setAttribute('alt', titleGif)
